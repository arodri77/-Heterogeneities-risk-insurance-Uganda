...
panelcontrol = panelbal.merge(control, on="hh", how='outer')
panelcontrol.set_index(["hh","wave"],inplace=True)

panelcontroldiff = panelbaldiff.merge(control, on="hh", how='outer')
...
print(test5.summary())
test6 = sm.ols("Δc ~ Δavgc +Δy*female +female +Δy*urban +urban +Δy*c2 +c2 +Δy*c3 +c3 +Δy*c4 +c4 +Δy*c5 +c5 +region2 +region3 +region4 +Δy*region2 +Δy*region3 +Δy*region4 ", data=data).fit()
print(test6.summary())

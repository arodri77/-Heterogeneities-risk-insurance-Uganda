...
lab99["hh"] = pd.to_numeric(np.array(lab99.index.values))
summaryw = lab99.describe()/dollars

counthh= pd.value_counts(lab99["hh"])
...
bus12["hh"] = pd.to_numeric(np.array(bus12.index.values))
...
other = other[["HHID","h11aq05","h11aq06"]]
...
other = other
other["hh"] = pd.to_numeric(np.array(other.index.values))
...
counthh= pd.value_counts(income_gsec["hh"])

income_gsec.to_csv('income_hhsec_2009.csv', index=False)